...
### Встроенное логирование модулей

Проект использует библиотеку `logging` для автоматического логирования работы модулей `masks` и `utils`. Все операции и ошибки записываются в файлы логов.

#### Расположение файлов логов

Логи сохраняются в папку `logs/` в корне проекта:
- `logs/masks.log` - логи работы модуля маскировки карт и счетов
- `logs/utils.log` - логи работы модуля загрузки транзакций

#### Формат записей в логах

Каждая запись в логе имеет следующий формат:
```
YYYY-MM-DD HH:MM:SS - имя_модуля - УРОВЕНЬ - сообщение
```

**Примеры записей:**
```
2025-11-18 22:50:51 - masks - INFO - Номер карты успешно замаскирован: 1234 56** **** 3456
2025-11-18 22:50:59 - masks - ERROR - Ошибка при маскировке номера карты 123: Длина номера карты должна быть от 13 до 19 цифр
2025-11-18 22:50:58 - utils - WARNING - Файл не найден: nonexistent_file.json
2025-11-18 22:50:51 - utils - INFO - Успешно загружено 101 транзакций из файла: data/operations.json
```

#### Уровни логирования

- **INFO** - информационные сообщения о выполненных операциях
- **WARNING** - предупреждения (например, файл не найден)
- **ERROR** - ошибки валидации и исключения
- **DEBUG** - отладочная информация (не записывается, уровень установлен на INFO)

#### Особенности

- Логи **перезаписываются** при каждом запуске приложения (режим `'w'`)
- Папка `logs/` создается автоматически при первом использовании
- Кодировка файлов: UTF-8
- Логирование работает автоматически, дополнительная настройка не требуется
...
│   └── logger_config.py  # Настройка логирования для модулей
├── generators/
...
│   ├── masks.log         # Логи модуля маскировки (создается автоматически)
│   └── utils.log         # Логи модуля загрузки транзакций (создается автоматически)
...
| Файл                                | Описание                                                            |
|-------------------------------------|---------------------------------------------------------------------|
| `src/masks.py`                      | Базовые функции маскировки карт и счетов (с встроенным логированием)|
...
| `src/utils.py`                      | Функции для загрузки транзакций из JSON-файлов (с встроенным логированием)|
| `src/external_api.py`               | Функции для конвертации валют через внешний API                     |
| `src/logger_config.py`              | Настройка логирования для модулей masks и utils                     |
...
| `logs/masks.log`                    | Файл логов модуля маскировки (создается автоматически)              |
| `logs/utils.log`                    | Файл логов модуля загрузки транзакций (создается автоматически)     |
