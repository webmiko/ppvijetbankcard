...
class TestMaskAccountCardFixtures:
    ...
    def test_mask_account_card_card(self, sample_card_strings: List[str]) -> None:
        """Проверка маскирования номера карты."""
        assert mask_account_card(sample_card_strings[0]) == "Visa Platinum 7000 79** **** 6361"

    def test_mask_account_card_account(self, sample_card_strings: List[str]) -> None:
        """Проверка маскирования номера счета."""
        assert mask_account_card(sample_card_strings[3]) == "Счет **4305"

    def test_mask_account_card_card_single_word(self, sample_card_strings: List[str]) -> None:
        """Проверка маскирования карты с одним словом в названии."""
        assert mask_account_card(sample_card_strings[1]) == "Maestro 7000 79** **** 6361"

    def test_mask_account_card_card_multiple_words(self, sample_card_strings: List[str]) -> None:
        """Проверка маскирования карты с несколькими словами в названии."""
        assert mask_account_card(sample_card_strings[4]) == "Счет **3456"

    def test_mask_account_card_account_with_spaces(self, sample_card_strings: List[str]) -> None:
        """Проверка маскирования счета с пробелами."""
        assert mask_account_card(sample_card_strings[4]) == "Счет **3456"

    def test_mask_account_card_invalid_format(self, sample_card_strings: List[str]) -> None:
    ...
    def test_mask_account_card_empty_string(self, sample_card_strings: List[str]) -> None:
...
class TestGetDateFixtures:
    ...
    def test_get_date_empty_string(self, sample_dates: Dict[str, str]) -> None:
        """Проверка вызова исключения при пустой строке."""
        with pytest.raises(ValueError, match="Неверный формат даты"):
            get_date(sample_dates["empty"])

    def test_get_date_invalid_string(self, sample_dates: Dict[str, str]) -> None:
