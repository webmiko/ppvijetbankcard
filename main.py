...
def demo_logging() -> None:
    """Демонстрация работы встроенного логирования модулей masks и utils."""

    print("\n" + "=" * 60)
    print("Демонстрация встроенного логирования модулей:")
    print("=" * 60)

    print("\nЛогирование модуля masks:")
    print("-" * 60)
    print("Все операции маскировки карт и счетов автоматически логируются в файл logs/masks.log")
    print("\nПримеры операций, которые будут залогированы:")

    # Демонстрация успешных операций
    print("\n1. Успешная маскировка карты:")
    try:
        card_result = get_mask_card_number("1234567890123456")
        print(f"   Результат: {card_result}")
        print("   → В лог записано: INFO - Номер карты успешно замаскирован")
    except ValueError as e:
        print(f"   Ошибка: {e}")

    print("\n2. Успешная маскировка счета:")
    try:
        account_result = get_mask_account("12345678901234567890")
        print(f"   Результат: {account_result}")
        print("   → В лог записано: INFO - Номер счета успешно замаскирован")
    except ValueError as e:
        print(f"   Ошибка: {e}")

    # Демонстрация обработки ошибок
    print("\n3. Обработка ошибок:")
    print("   Попытка маскировки невалидного номера карты:")
    try:
        get_mask_card_number("123")
    except ValueError as e:
        print(f"   Ошибка перехвачена: {e}")
        print("   → В лог записано: ERROR - Ошибка при маскировке номера карты")

    print("\nЛогирование модуля utils:")
    print("-" * 60)
    print("Все операции загрузки транзакций автоматически логируются в файл logs/utils.log")
    print("\nПримеры операций, которые будут залогированы:")

    # Демонстрация успешной загрузки
    print("\n1. Успешная загрузка транзакций:")
    transactions = load_transactions_from_json("data/operations.json")
    print(f"   Загружено транзакций: {len(transactions)}")
    print("   → В лог записано: INFO - Успешно загружено N транзакций из файла")
    ...
    print(f"   Результат: {empty_result} (пустой список)")
    print("   → В лог записано: WARNING - Файл не найден")

    print("\n" + "=" * 60)
    print("Информация о логах:")
    print("=" * 60)
    print("\nФайлы логов находятся в папке logs/:")
    print("  - logs/masks.log - логи модуля маскировки карт и счетов")
    print("  - logs/utils.log - логи модуля загрузки транзакций")
    print("\nФормат записей в логах:")
    print("  YYYY-MM-DD HH:MM:SS - имя_модуля - УРОВЕНЬ - сообщение")
    print("\nПримеры записей:")
    print("  2025-11-18 22:50:51 - masks - INFO - Номер карты успешно замаскирован: 1234 56** **** 3456")
    error_example = (
        "  2025-11-18 22:50:59 - masks - ERROR - Ошибка при маскировке номера карты 123: "
        "Длина номера карты должна быть от 13 до 19 цифр"
    )
    print(error_example)
    print("  2025-11-18 22:50:58 - utils - WARNING - Файл не найден: nonexistent_file.json")
    print("\nУровни логирования:")
    print("  - INFO: информационные сообщения о выполненных операциях")
    print("  - WARNING: предупреждения (например, файл не найден)")
    print("  - ERROR: ошибки валидации и исключения")
    print("\nОсобенности:")
    print("  - Логи перезаписываются при каждом запуске приложения")
    print("  - Папка logs/ создается автоматически при первом использовании")
    print("  - Кодировка файлов: UTF-8")
    print("  - Логирование работает автоматически, дополнительная настройка не требуется")
    print("\nПроверьте файлы logs/masks.log и logs/utils.log для просмотра всех записей!")
